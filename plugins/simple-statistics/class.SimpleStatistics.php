...
class SimpleStatistics {
    ...
    public function collectData(array $args): array {
        ...
        $count = $this->get('pageviews', date('Y-m-d'), $this->Wcms->currentPage);
        if (! $count) {
            $count = 0;
        }
        $count++;
        $this->set('pageviews', date('Y-m-d'), $this->Wcms->currentPage, $count);

        $count = $this->get('sessions', date('Y-m-d'), session_id());
        if (! $count) {
            $count = 0;
        }
        $count++;
        $this->set('sessions', date('Y-m-d'), session_id(), $count);
    ...
    public function alterAdmin(array $args): array {
        ...
        // Sessions today
        $count = @$this->get('sessions', date('Y-m-d'));
        ...
        // Most popular page
        $pages = (array) @$this->get('pageviews', date('Y-m-d'));
        asort($pages);
        ...
        $total = 0;
        for ($i = 0; $i < 7; $i++) {
            $pages = @$this->get('pageviews', date('Y-m-d', strtotime("last monday + $i days")));
            if (! $pages) {
                $pages = [];
            }
            $count = 0;
        ...
        $total = 0;
        for ($i = 0; $i < 7; $i++) {
            $count = @$this->get('sessions', date('Y-m-d', strtotime("last monday + $i days")));
            $count = count((array) $count);
            if (! $count) {
                $count = 0;
            }
        ...
        $total = [];
        for ($i = 0; $i < 7; $i++) {
            $pages = (array) @$this->get('pageviews', date('Y-m-d', strtotime("last monday + $i days")));
